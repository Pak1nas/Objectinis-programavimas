...
void spausdinti_lentele_ekrane(const vector<stud>& A){
...
void spausdinti_lentele_file(const vector<stud>& A){
    
    ofstream ats("rezultatas.txt");
    if(!ats){
        cout<<"nepavyko sukurti failo!"<<endl;
        return;
    }
    
    ats<<endl<<left<<setw(15)<<"Pavarde"<<left<<setw(15)<<"Vardas"<<right<<setw(20)<<"Galutinis (Vid.)"<<right<<setw(20)<<"Galutinis (Med.)"<<endl;

    ats<<string(70, '-')<<endl;

    for(const auto &s : A){
        ats<<left<<setw(15)<<s.pavarde<<left<<setw(15)<<s.vardas<<right<<setw(20)<<fixed<<setprecision(2)<<s.vid<<right<<setw(20)<<fixed<<setprecision(2)<<s.med<<endl;
    }
    ats.close();
}
...
int main() {
    ...
    cout<<"1-Duomenys ekrane"<<endl;
    cout<<"2-Duomenys file"<<endl;
    cout<<"Pasirinkite 1"<<endl;
    ...
    if(h==1){
        spausdinti_lentele_ekrane(A);
    }
    else {
        spausdinti_lentele_file(A);
    }

    auto end = chrono::high_resolution_clock::now();
    chrono::duration<double> diff = end - start;
    cout << "Operacija uztruko: " << diff.count() << " s" << endl;
